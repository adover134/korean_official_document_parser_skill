...
- 8GB급 VRAM 환경에서는 문서당 후보가 많을 경우(체크리스트/표가 많은 문서) 자동으로 70개 단위로
  분할 호출한다 — 컨텍스트 제한이 다른 환경이면 소스 수정 없이 `--max-candidates-per-call`로
  시작 크기를 직접 조정할 수 있다. Groq처럼 TPM 한도가 낮은 클라우드 백엔드는 이제 실패 시(제공자
  API가 실제로 알려준 한도 기준) 자동으로 배치를 줄여 재시도하므로 보통 직접 조정할 필요는 없다
  (2026-08-27, `llm_backend.py`의 `RateLimitExceeded` 참고).
